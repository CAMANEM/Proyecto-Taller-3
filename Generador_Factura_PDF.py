...
servicios = [{"ID": ID, "tipo": descripcion, "cant": cant},
...
class Factura:
    ...
    def crear_plantilla(self):
        ...
        for i in range(0, 10):  # Crea las lineas de la tabla de la factura
        ...
        self.pdf.linkURL("https://tecdigital.tec.ac.cr/dotlrn/file-storage/view/dotlrn_fs_77630404_root_folder/dotlrn_fs_77630404_shared_folder/pagina/index.html", (420, 610, 480, 630), relative=1) #  Enlace en los términos de factura
    ...
    def facturar(self):
        ...
        #  Montos
        self.y = 505
        for servicio in self.servicios: # Escribe cada servicio que se listó para ser facturado, hasta un máximo de 10 servicios por factura.
            if self.contador < 10:
                #print(servicio)
                ...
                break
        # Calcula y escribe el subtotal, IVA y Total de la factura
        self.pdf.drawRightString(505, 308, str(self.subtotal))
        self.precio = int(self.subtotal * 0.13) # i.v.a
        ...
        self.pdf.drawRightString(440, 268, self.moneda)
        self.pdf.save()
...
class Cambio_Moneda(Factura):
    ...
    def __init__(self, precio_dolar, n_factura):
        ...
        for i in data["Facturas"]: #  Busca la factura a modificar
            if i["N°_Factura"] == n_factura:
                if i["Moneda"] == "C":  ##Si la factura seleccionada se encuentra en colones, se realiza el cambio de moneda y modificacion de la factura
                    ...
                    self.cambia_costos(precio_dolar)  ## Cambia los precios de los servicios a dolares
                    self.crear_plantilla()  ## Crea la plantilla de la factura
                    self.facturar()  ## Genera y reemplaza la factura deseada
                    self.sobreescribir_factura(n_factura)  ## Sobreescribe los datos almacenados sobre la factura
                    break

                else:
                    messagebox.showinfo("Error", "La factura seleccionada ya se encuentra en dolares")
                    break
    ...
    def cambia_costos(self, precio_dolar):
        for i in self.costo["ID_existentes"]: ## Realiza la conversion de colones a dolares a todos los servicios, de dicha factura
            self.costo["Servicios"][i]["costo"] = str(int(int(self.costo["Servicios"][i]["costo"]) / precio_dolar))

    def sobreescribir_factura(self, n_factura): ## Sobreescribe los datos guardados sobre la factura
        self.facturas = json.load(open("Servicios/Facturas.json"))
        indice = 0
        for i in self.facturas["Facturas"]:
            if i["N°_Factura"] == n_factura:
                self.facturas["Facturas"][indice] = {"N°_Factura": n_factura,  # N° de factura
                                               "Fecha": self.fecha,
                                               "Vencimiento": self.vence,
                                               "Cliente": self.usuario[0],
                                               "Cedula": self.usuario[1],
